...
\begin{enumerate}
\item Robot con ruedas atrás y dos puntos de apoyo.
\item Robot con ruedas en el medio y cuatro puntos de apoyo (robot simétrico).
\end{enumerate}

En \cite{rcva_10x} se estudia el valor máximo aceleración durante las fases de aceleración y frenado de un robot de estas características utilizando los los modelos dinámicos de la siguiente figura:


\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{dinamica_a_max}
\caption[]{Modelo dinámico fase de aceleración y frenado para robots con ruedas atrás y en el medio.}
\label{fig:dinamica_a_max}
\end{figure}
...
A partir del modelo de la figura \ref{fig:dinamica_a_max} y siguiendo un análisis igual al realizado para robots con ruedas en un extremo en \cite{rcva_10x} se obtienen las ecuaciones que definen las aceleraciones máximas para el caso del robot simétrico. Debido a que el robot tiene 4 puntos de apoyo no es posible que el robot haga \emph{el caballito}, la aceleración máxima vendrá dada únicamente para la condición de derrape de las ruedas y es la misma para las fases de aceleración y frenado dada la simetría del robot:

 
\begin{equation}
\label{eq:a_max_simetrico}
\boxed{a_{max} = g\,\frac{Ka}{1 + Ka\,(h/d)}}
\end{equation}
