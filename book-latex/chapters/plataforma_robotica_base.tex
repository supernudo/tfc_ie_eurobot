...
Siendo:
\begin{description}
\item $Fn$: fuerza normal o vertical del suelo (opuesta a la fuerza de apoyo debida al peso del robot)
\item $Fx$: fuerza de propulsión de en la rueda
...
Por otro lado, el valor de la aceleración máxima para cada condición depende del coeficiente de adherencia de las ruedas ($Ka$) y la posición del centro de gravedad del robot, respecto al eje motriz y los puntos de apoyo. 
...
a_{max1} &= g\,\frac{Ka \, d2}{d-(Ka \, h)} \label{eq:a_max_aceleracion_derrape}\\
...
Fase de frenado ($a<0$):
\begin{empheq}[box=\fbox]{align}
a_{max1} &= g\,\frac{Ka \, d2}{d+(Ka \, h)} \label{eq:amax_frenado_derrape}\\
...
Como se ha visto, las ruedas del eje motriz se encuentran conectadas al motor por medio de una reductora. La figura \ref{fig:} representa el modelo mecánico de uno de los motores. 
...
Observar que la relación entre potencias en \eqref{eq:relacion_P2_P1} no depende de $K$. Si $K$ aumenta el par de la rueda $M2$ aumenta pero la velocidad $w2$ disminuye.

Por otro, del modelo en régimen permanente de un motor de corriente contínua, se sabe que la tensión de armadura del motor $U$ y el par motor $M$ se corresponden con:
\begin{align}
U &= r\,i + Kv\,\omega \label{eq:tension_armadura} \\
M &= Km\,i \label{eq:par_motor}
\end{align}
...
\item $Kv:$ constante de fuerza electromecánica (V/(rad/s))
\item $Km$: constante de par motor (Nm/A)
\end{description}

A la hora de dimensionar la reductora se busca determinar el valor de reducción de la misma ($K$) que genere un par suficiente en la rueda $M3$ para vencer las fuerzas aplicadas en la misma que se oponen al movimiento (ver \ref{fig:dinamica_a_max_ruedas_atras}).

No obstante existe una condición más restrictiva a la hora de dimensionar la reductora. Es el caso en el que las ruedas queden bloqueadas debido a que el robot a dado con un obstáculo imprevisto. Por ejemplo una pared debido a un error de posicionamiento, o que haya un objeto inesperado entre el robot y la pared. En ese caso el controlador de posición reacciona aumentando la tensión del motor hasta su valor máximo $U_{max}$ y se corre el riesgo de que el motor se queme. Este caso es analizado en \ref{rcva_10x} a partir de modelo térmico del motor. El análisis determina que en caso de bloqueo de una rueda la vida útil del motor es de 2,3s, a menos que el driver del motor sirva de fusible y se rompa antes o que exista un mecanismo de protección.
...
Por un lado se calcula el par en el motor $M1$ necesario para producir deslizamiento a partir del par en la rueda $M3$, el cual depende de a su vez la fuerza de propulsión máxima de las ruedas $Fx_{max}$.
\begin{align}
\end{align}

La fuerza aplicada sobre las ruedas $F1$ depende de la distribución de pesos sobre el eje motriz y se corresponde con el peso $P1$ medido durante el la medida experimental del coeficiente de adherencia $Ka$.
\begin{align}
\end{align}

Sustituyendo 3 en 2 en resultado en 1 se obtiene el par del motor $M1$ necesario en el límite del deslizamiento.
...
El valor de la reductora mínima $K_{min}$ se obtiene despejando de (4):
\begin{equation}
\end{equation}


Notar que $K_{min}$ aumenta si los términos $Ka\,F1$, $D$ o $r$ aumentan, o $Km$, $U_{max}$ disminuye.
