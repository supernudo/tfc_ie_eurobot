...
En el año 2010 se desarrolló una plataforma robótica base, cuyo diseño fue reutilizado y mejorado en el desarrollo de robots posteriores. Dicha plataforma implementa las funcionalidades básicas de un robot de Eurobot vistas en el capitulo \ref{cha_el_robot_de_eurobot}:
...
En las secciones de este capítulo se aborda la implementación mecánica, hardware y software relacionada con las funcionalidades básicas mencionadas, así como los fundamentos implícitos en ellas que se han considerado oportunos.  


\section{Descripción general de la plataforma robótica}

La funcionalidad de \textbf{desplazamiento y localización} de la plataforma robótica, se implementa a partir de un \emph{sistema de tracción diferencial} y un \emph{sistema de posicionamiento por odometría}. Mediante la unión de estos dos sistemas se implementa a su vez un \emph{control de posición polar} del robot, a partir del cual, se gestiona la \emph{generación de trayectorias}.
...
El hardware de la plataforma robótica lo compone principalmente una tarjeta de alimentación, la tarjeta \emph{Eurobotics 2010} y una tarjeta de expansión de entradas y salidas. La tarjeta \emph{Eurobotics 2010} tiene la capacidad de gestionar e implementar los sistemas mencionados anteriormente, así como los sistemas mecánicos específicos de la prueba de Eurobot, destinados a la manipulación de elementos de juego.

Respecto a la \textbf{evitación de obstáculos}, esta se realiza a partir de sensores de distancia situados en las caras del robot (ver figura \ref{fig_plataforma_partes_2}) y un sistema específicamente desarrollado para la detección de la posición del oponente. Este sistema hace uso de los soportes y espacios destinados a sistemas de balizas. Concretamente, el sistema desarrollado está basado en un sensor tipo faro situado en el robot y balizas reflectantes situadas en el oponente (ver figura \ref{fig_plataforma_partes_2}). El sensor tipo faro es un sistema totalmente autónomo que se comunica mediante un enlace \emph{Bluetooth} con la tarjeta \emph{Eurobotics 2010}.
...
La implementación de todos los sistemas y funcionalidades mencionadas de la plataforma robótica ha sido realizada a partir de las librerías Aversive4dspic. Estas librerías han sido migradas de las librerías Aversive \cite{aversive_src} (microcontroladores AVR) a microcontroladores dsPIC y PIC24, utilizados por la tarjeta \emph{Eurobotics 2010}. Estas librerías constituyen una plataforma software para la implementación de sistemas de control y de robots.

Por último, utilizando la interfaz \emph{Bluetooth} de la tarjeta \emph{Eurobotics 2010} se implementa la funcionalidad de \textbf{comunicación entre dos robots}: uno principal y uno secundario. En este caso, la arquitectura mecánica, hardware y software del robot principal es duplicada en el robot secundario, y la comunicación entre ambos se realiza mediante un enlace serie \emph{Bluetooth} entre las tarjetas \emph{Eurobotic 2010}.


%\section{Sistemas mecánicos}
...
La gráfica de la figura \ref{fig_perfil_vmax_amax} muestra el resultado de aplicar un perfil trapezoidal cuya velocidad es la máxima. Observar que al final de la fase de aceleración la tensión de control de los motores se encuentra muy cerca de su valor de saturación (63500). Si se disminuye la velocidad hasta $0,8m/s$, el valor de la tensión baja hasta el $80\%$ de su máximo (ver figura \ref{fig_perfil_v08_amax}).
...
\hline
Distancia 	& $d$ 		& $m$  			& $\text{pulsos}$ \\
Velocidad 	& $v$		& $m/s$			& $\text{pulsos}/T_s$\\
Aceleración & $a$		& $m/s^2$		& $\text{pulsos}/T_s^2$ \\
\hline
\end{tabular}
\end{table}
...
\subsection{Sistema de generación de trayectorias}
\subsection{Módulo de gestión de trayectorias\\ \texttt{trayectory\_manager}}
\subsection{Módulo de gestión de la posición\\ \texttt{position\_manager}}
\subsection{Módulo de gestión de controladores en lazo cerrado\\ \texttt{control\_system\_manager}}
\subsection{Módulo de detección de bloqueos mecánicos\\ \texttt{blocking\_detection\_manager\_manager}}
\subsection{Modulo de evitación de obstáculos\\ \texttt{obstacle\_avoidance}}
