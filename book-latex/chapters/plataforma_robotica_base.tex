...
El análisis parte del perfil perfil de velocidad representado en la figura \ref{fig:} y de las ecuaciones que describen el modelo del motor de continua
\begin{align}
...
Por otro lado, hay que recordar que el calculo de la reductora también se realiza en base a la tensión de saturación $v_{sat}$, la cual, una vez alcanzada haría deslizar las ruedas para evitar el bloque del motor. Es por ello que el instante $t1$ constituye el punto más crítico de las fases del perfil trapezoidal.

El límite de funcionamiento sin saturación es dado por la ecuación \eqref{eq:tension_armadura2}, valida en el instante $t1$. Por un lado, la corriente de armadura se puede expresar en función de la aceleración $a$ del robot del siguiente modo:
\begin{equation}
\begin{split}
i &= \frac{M1}{K_m}\\
M1 &= \frac{M3}{K\,Rd}\\
...
Dicha recta se representa en la figura \ref{fig:recta_v_func_a}, donde se puede observar que la aceleración máxima $a_{max}$ define la zona útil de funcionamiento sin saturación y deslizamiento. 
...
Por ejemplo, para el reductora, se obtiene los resultados de la figura \ref{fig:dimensionamiento_motor_ptinto}. Se fija una aceleración $a = 0,8\,a_{max} = $
