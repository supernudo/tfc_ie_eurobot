...
\label{sec_partes_plataforma_robotica}
...
El sistema de evitación de obstáculos está compuesto por varios sensores de corto alcance situados en las caras del robot (ver figura \ref{fig_plataforma_partes_2}) y un sistema de detección del oponente de largo alcance basado en un sensor tipo faro y balizas reflectantes (ver figura \ref{fig_plataforma_partes_2}).
...
\label{sec_sitema_posicionamiento_relativo}

La plataforma robótica desarrollada implementa un control de la posición del robot. La medida de la posición del robot se realiza mediante dos ruedas libres unidas a sensores tipo encoders. A través de la medida de posición se implementa indirectamente el control de posición de los motores del sistema de tracción y de forma directa el sistema de posicionamiento por odometría del robot. Esta configuración difiere del esquema clásico en el que el encoder se encuentra conectado directamente al eje del motor y permite reducir los errores sistemáticos de odometría \cite{bor96_cap5} y que un posible deslizamiento o derrape de las ruedas motrices no afecte al control de posición del robot.
...
\label{sec_control_posicion_tipo_polar}
...
\label{sec_deteccion_bloqueos_mecanicos}



\subsection{Posicionamiento relativo mediante odometría}
\label{sec_posicionamiento_relativo_mediante_odometria}
...
\label{sec_sistema_deteccion_obstaculos}
...
\label{sec_generacion_de_trayectorias}
