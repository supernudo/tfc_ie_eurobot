...
M1 &= \frac{M3}{K\,Rd}\\
M3 &= Fx\,\frac{D}{2}\\
2Fx &= m\,a \label{eq:desarllo_i_func_a}
\end{split}
\end{equation}
...
Sustituyendo en \eqref{eq:tension_armadura2} se obtiene\footnote{$K_v$ ha sido sustituido por $K_m$ ya que coinciden si ambas constantes se expresan en $[V/(rad/s)]$ y $Nm/A$, respectivamente}:
\begin{equation}
U = (\frac{r\,m\,D}{4K\,Rd\,K_m})\,a + (\frac{2K_m\,K}{D})\,v \label{u_func_a_v}
\end{equation}
...
Observar la segunda recta de la figura \ref{fig:recta_velocidad_maxima_ptinto}, correspondiente a un motor de la mitad de potencia. Para una misma aceleración la velocidad es menor. En el primer caso la velocidad es superior a la velocidad deseada ($1 m/s$) y el motor BG40x50 es válido, mientras que en el segundo caso el motor no sería valido.
