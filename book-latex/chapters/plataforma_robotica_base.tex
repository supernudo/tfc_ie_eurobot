...
En el año 2010 se desarrollo una plataforma robótica base, cuyo diseño fue reutilizado en el desarrollo de robots posteriores. Como se ha visto en la sección \ref{sec:partes_robot_eurobot}, capitulo \ref{cha:el_robot_de_eurobot}, las funcionalidades y partes de esta plataforma no dependen de la temática del juego del Eurobot. Así la plataforma desarrollada implementa las siguientes funcionalidades básicas de un robot de Eurobot:
...
Cada una de estas partes implicó un desarrollo mecánico, hardware y software, o alguno de ellos.
...
Los puntos de apoyo utilizado normalmente son de tipo \emph{rollon}, en caso de no disponer espacio suficiente también es posible utilizar materiales de bajo coeficiente de rozamiento como el teflón como muestra la figura \ref{fig:sistema_traccion_rollones}.
...
Mecánicamente el conjunto de los motores, reductora y ruedas forma lo que se denomina \emph{bloque motor}. El bloque motor se reutiliza como una parte independiente en cada diseño de forma que se sitúa sobre la base del robot adaptándose al resto de la mecánica del robot.


\subsection{Dinamica básica}
\label{sec:dinamica_basica}

Es importante entender la dinámica de un robot de Eurobot y tenerla en cuenta durante el diseño mecánico del mismo. El movimiento más común de un robot de Eurobot son los desplazamientos entre dos puntos. Durante dichos desplazamientos el robot parte de parado, recorre una distancia y vuelve a detenerse. Así, a cada desplazamiento le corresponde una fase de aceleración, velocidad constante y otra de frenado.
...
La aceleración máxima vendrá dada por la fuerza de propulsión de las ruedas máxima $Fx_{max}$ a partir de la cual se produce el deslizamiento de las ruedas. Por lo tanto ha de cumplirse la condición
...
El coeficiente de adherencia permite calcular, junto con la posición del centro de gravedad del robot, la aceleración máxima del robot. El valor del coeficiente de adherencia se puede medir experimentalmente mediante el método propuesto por Jacques Coulon en \cite{rcva_10x}, conocido como el \emph{método del cubo}. 
...
\begin{enumerate}
\item Apoyar las ruedas del robot sobre una báscula manteniendo el robot horizontal. Ayudarse de un soporte sobre los puntos de apoyo del robot. El peso medido $P1$ se corresponde con la fuerza con la que las ruedas apoyan en el suelo.
