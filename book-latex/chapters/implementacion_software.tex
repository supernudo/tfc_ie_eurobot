...
La gestión de trayectorias utiliza los módulos \texttt{obstable_avoidance} y \texttt{trajectory_manager}:

\begin{description}
\item \textbf{\textttt{trajectory_manager}}:

A partir de un punto de destino del robot y de la posición actual del mismo, este módulo calcula la trayectoria para alcanzar dicho punto. El fin de la trayectoria se obtiene mediante la lectura del estado de la misma. Para alcanzar el punto de destino, pueden ser necesarios varios movimientos, las correspondientes a consignas de posición de estos son procesadas por el bloque de control de posición.

\item \textbf{\textttt{obstacle_avoidance}}:

Este módulo permite a partir de un punto de destino del robot y una lista de obstáculos (representados por polígonos) obtener el camino más corto para llegar a dicho punto. Como resultado el módulo devuelve la lista de puntos que forman el camino a seguir.
\end{description}

Estos dos módulo permiten por ejemplo, implementar a nivel de temática del juego una función que permita llevar al robot a un punto determinado del campo. La temática de juego conoce el campo de juego, la posición de sus elementos y puede acceder a la información de la posición de los opoenentes dada por la baliza. la función \texttt{goto_avoid(x,y)} a partir de la cual


 El primero permite a a partir de un punto de destino, calcular el camino más corto evitando los obstáculos del campo y a los oponentes. Dicho camino se representa por una lista de puntos por los que el robot ha de pasar. El modulo \texttt{trajectory_manager} permite llevar al robot hacia un punto determinado. 


En esta caso la figura \ref{fig_sw_diagrama_plataforma_robotica} representa el funcionamiento de 


La lectura de los encoders de las ruedas libres del robot y el control de los motores de tracción es realizado por el control de posición a través de la capa de drivers HW. 
