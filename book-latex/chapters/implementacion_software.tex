...
\item Reduce el tiempo de puesta en marcha de la estratégica y simular todo tipo de situaciones.

\item Permite probar cambios de estrategia de ultimo momento.
...
\begin{itemize}
\item El funcionamiento simulado de elementos externos como la baliza o la comunicación con el robot secundario ha de ser de igual comportamiento. En caso contrario las pruebas realizadas en simulación no se correspondan con pruebas reales.

\item Tendencia a confiar demasiado en la simulación. Aunque la simulación de los resultados esperados es necesario validarlos en pruebas reales.
\end{itemize}

El simulador desarrollado permite simular el funcionamiento del robot y el campo de juego. Está basado en el simulador desarrollado por el equipo Microb Technology \cite{microb} y utiliza la capacidad de las librerías Aversive4dspic para ser compiladas para un \emhp{host GNU/Linux}.

La simulación de algunas funcionalidades de las librerías Aversive4dspicse se realiza mediante el módulo \texttt{hostsim} (de \texttt{modules/base}). Este módulo simula la salida estándar del modulo de comunicaciones \texttt{uart}, la medida de tiempo y la ejecución periódica del gestor de eventos, mediante una implementación mediante recursos propios del sistema GNU/Linux. Por otro lado
...
\item Sistemás mecánicos 

La emulación de sistemas mecánicos consiste se realiza mediante un retardo que equivalente al tiempo que los mecanismos tardan en realizar una función.
