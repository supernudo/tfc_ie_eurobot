...
Pues bien este proyecto no es una excepción y también camina a hombros de gigantes. El código fuente de los microcontroladores de algunos de los robots desarrollados (Husillo, Joselito o Campanolo) fue escrito desde cero. Otros se apoyaron en el código escrito en años anteriores (Electrococo, Mr.Proper y Topolino). Y finalmente a partir del año 2010 los robots (Trompetero, Zamorano, Automático, Crispin, P.Tinto y Tirantes) se apoyaron sobre código abierto escrito por terceros.

El utilizar código abierto de terceros vino dado por el objetivo de evolucionar tecnológicamente los robots a desarrollar para Eurobot. Para ello se estudió los fundamentos técnicos y tecnológicos de los robots hasta entonces que obtenían muy buenos resultados (equipos como RCVA\cite{rcva}, Microb Tecnology\cite{microb}, RCA Archen\cite{rca_aachen} o Turak\cite{turak}). Al final de dicho estudio se establecieron los requisitos de la mecánica, hardware y software necesarios, y se desarrollo una plataforma robótica base (el \emph{Dummy robot}) con una mecánica y una electrónica de acuerdo a los requisitos buscados.
...
La decisión de utilizar estas librerías y portarlas permitió no solo aprender a utilizarlas desde la capa de aplicación (el robot) sin no estudiar como estaban implementadas y adaptarlas para ser compatibles con una arquitectura diferente. En definitiva se reutilizó el código de terceros, lo cual implica mucho más que el simple hecho de aprovechar el trabajo realizado por otros. En este caso las librerías Aversive\cite{aversive_src} son unas librerías libres desarrolladas por varias personas, versionada bajo un sistemas de control de versiones,  bien estructuradas, escritas siguiendo una guía de estilo común, que explota al máximo el lenguaje ANSI C, que cuenta con un \emph{toolchain} que utiliza herramientas libres y que implementan todo tipo de funcionalidades y muchas de ellas dirigidas al desarrollo de robots como los de Eurobot. La migración de las librerías Aversive se realizó siguiendo y manteniendo toda las características anteriormente mencionada, lo cual implicó el aprendizaje de las mismas y de todos los conceptos derivados y subyacentes.     
...
Todo el trabajo realizado en el desarrollo de los diferentes robots viene principalmente motivado por la afición a la robótica, la electrónica, los sistemas embebidos y por una necesidad de inventar y crear cosas.
